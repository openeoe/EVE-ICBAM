...
fname = '/home/smarteco/DATA/forecast/' + sys.argv[1] + '/mem/' + sys.argv[2]
...
X = tf.placeholder(tf.float64, [None, num_periods, inputs])
y = tf.placeholder(tf.float64, [None, num_periods, output])

basic_cell = tf.nn.rnn_cell.BasicRNNCell(num_units=hidden, activation=tf.nn.relu)
rnn_output, states = tf.nn.dynamic_rnn(basic_cell, X, dtype=tf.float64)
